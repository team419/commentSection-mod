...
export default class HelpfulBtn extends React.Component {
  ...
  handleClickUseful() {
    const { buttonUsefull, showUseful } = this.state;
    this.setState({
      value: 'Thanks for your vote!',
      buttonUsefull: !buttonUsefull,
      showUseful: !showUseful,
    });
  }

  handleClickFunny() {
    const { buttonFunny, showFunny } = this.state;
    this.setState({
      value: 'Thanks for your vote!',
      buttonFunny: !buttonFunny,
      showFunny: !showFunny,
    });
  }

  handleClickCool() {
    const { buttonCool, showCool } = this.state;
    this.setState({
      value: 'Thanks for your vote!',
      buttonCool: !buttonCool,
      showCool: !showCool,
    });
  }
  ...
  render() {
    const {
      showUseful, showFunny, showCool, value, buttonUsefull, buttonFunny, buttonCool,
    } = this.state;
    const plusOneUseful = showUseful ? ' 1' : '';
    const plusOneFunny = showFunny ? ' 1' : '';
    const plusOneCool = showCool ? ' 1' : '';

    return (
      <div>
        <div className={css['thankYou']}>{value}</div>
        <button
          type="submit"
          className={
            buttonUsefull ? css['buttonTrue'] : css['buttonFalse']
          }
        ...
        <button
          type="submit"
          className={
            buttonFunny ? css['buttonTrue'] : css['buttonFalse']
          }
        ...
        <button
          type="submit"
          className={
            buttonCool ? css['buttonTrue'] : css['buttonFalse']
          }
